...
// Main Updated Route (Includes Paid Fines Registry logic)
app.get('/api/admin/dashboard-metrics', async (req, res) => {
    try {
        let pool = await sql.connect(config);
        ...
        // NEW: Database Layer Query to pull returned entries with accumulated fines
        let paidFinesResult = await pool.request().query(`
            SELECT t.student_id, s.name, t.book_id, CONVERT(VARCHAR, t.return_date, 105) AS return_date, t.fine
            FROM Transactions t 
            JOIN Student s ON t.student_id = s.student_id 
            WHERE LOWER(TRIM(t.status)) = 'returned' AND t.fine > 0
        `);
        ...
        return res.json({
            ...
            deptStats: deptStatsResult.recordset,
            defaulters: defaulters.recordset,
            paidFines: paidFinesResult.recordset
        });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ success: false, error: err.message });
