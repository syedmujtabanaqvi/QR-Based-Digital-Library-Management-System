...
// // 2. Get All Books from DB table 'Book2' (FIXED: Sub books dikhengi hamesha)
// app.get("/api/books", async (req, res) => {
//     try {
//         let pool = await sql.connect(config);
//         // ✅ SQL query saaf kar di taake out-of-stock books bhi dropdown mein nazar aayin
//         let result = await pool.request().query("SELECT * FROM Book2");
...
// // 5. Core Unified Business Logic & DB Book Issuance Controller (FIXED CODES)
// app.get('/:action/:book_id', async (req, res) => {
//     const { action, book_id } = req.params;
//     const { student_id } = req.query;
...
//         // ✅ FIXED VALIDATION: Ab check status par nahi balke quantity zero hone par block karega!
...
//         // ✅ FIXED QUERY 1: Safe SQL data update statements matching parameter bind IDs
//         await pool.request()
//             .input("id", sql.VarChar, book_id)
//             .query(`
//                 UPDATE Book2
...
//         // Database mein check karein ke is bache ki kitni transactions 'issued' status par hain
//         let result = await pool.request()
//             .input("student_id", sql.VarChar, student_id)
//             .query("SELECT COUNT(*) AS total_borrowed FROM Transactions WHERE student_id = @student_id AND status = 'issued'");
